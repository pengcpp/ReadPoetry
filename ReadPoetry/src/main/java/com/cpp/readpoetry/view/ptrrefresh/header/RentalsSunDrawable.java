...
public class RentalsSunDrawable extends Drawable implements Animatable {
    ...
    private void createBitmaps() {
        mSky = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.rentals_sky);
        mSky = Bitmap.createScaledBitmap(mSky, mScreenWidth, mSkyHeight, true);
        mTown = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.rentals_buildings);
        mTown = Bitmap.createScaledBitmap(mTown, mScreenWidth, (int) (mScreenWidth * TOWN_RATIO), true);
        mSun = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.rentals_sun);
    ...
    private void drawSky(Canvas canvas) {
        ...
        float offsetY = y + 50 + mSkyTopOffset // Offset canvas moving, goes lower when goes down
                - mSkyHeight * (skyScale - 1.0f) / 2 // Offset sky_ scaling, lower than 0, will go greater when goes down
    ...
    private void drawSun(Canvas canvas) {
        ...
        float offsetY = mSunTopOffset
                + (mTotalDragDistance / 2) * (1.0f - dragPercent); // Move the sun_ up
